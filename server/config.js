...
connection.query(tableTelephonie , (err, results) =>{
  if (err) throw err;
   ...
   for (let i = 0; i < createTelephonie.length; i++){
     connection.query(`SELECT * FROM si_sng.telephonie WHERE Code_telephonie = ${createTelephonie[i].Code_telephonie} AND Libelle_telephonie = "${createTelephonie[i].Libelle_telephonie}"`, (err, result) => {
       if (err) throw err;
      if (!result.length > 0) {
            connection.query(`INSERT INTO si_sng.telephonie (Code_telephonie, Libelle_telephonie) VALUES (
